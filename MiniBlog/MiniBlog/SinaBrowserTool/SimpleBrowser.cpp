...
void  SimpleBrowser::Navigate2(VARIANT *URL, VARIANT *Flags, VARIANT *TargetFrameName, VARIANT *PostData,VARIANT *Headers)
{
	if (!_Browser)
		return;

	_Browser->Navigate2(URL, Flags, TargetFrameName,PostData, Headers);
...
STDMETHODIMP SimpleBrowser::QueryInterface(REFIID iid,void**ppvObject)
{
	*ppvObject = NULL;
...
STDMETHODIMP_(ULONG) SimpleBrowser::AddRef()
{
	return ::InterlockedIncrement( &_refNum );
}

STDMETHODIMP_(ULONG) SimpleBrowser::Release()
{
	return ::InterlockedDecrement( &_refNum );
}
// IDispatch Methods

HRESULT _stdcall SimpleBrowser::GetTypeInfoCount(
	unsigned int * pctinfo) 
{
	return E_NOTIMPL;
}

HRESULT _stdcall SimpleBrowser::GetTypeInfo(
	unsigned int iTInfo,
	LCID lcid,
...
HRESULT _stdcall SimpleBrowser::GetIDsOfNames(
	REFIID riid, 
...
HRESULT _stdcall SimpleBrowser::Invoke(
	DISPID dispIdMember,
	REFIID riid,
	LCID lcid,
	WORD wFlags,
	...
	)
{
	...
	case DISPID_AMBIENT_DLCONTROL:
		...
		pVarResult->vt = VT_I4;
		pVarResult->lVal = DLCTL_NO_JAVA |   DLCTL_NO_RUNACTIVEXCTLS   |   DLCTL_NO_DLACTIVEXCTLS; 
		//DLCTL_NO_SCRIPTS;//DLCTL_DOWNLOADONLY;//DLCTL_DLIMAGES | DLCTL_VIDEOS | 
...
BOOL WeiboServiceBase1::GetSource(IHTMLDocument2 *pDoc2,CString& refString)
{
...
IDispatch * WeiboServiceBase1::getElementInCollection(IHTMLElementCollection   *pEltCollection,int   ndx) 
{ 
...
BOOL WeiboServiceBase1::GetCookie(IHTMLDocument2 *pDoc2,CString& refString)
{
	BSTR bstrCookie;
