...
CString VariantToString(VARIANT * va)
{
	...
	case VT_VARIANT | VT_BYREF:
		...
		//s.Format(_T("%s"), (BSTR)va->pbstrVal);
...
)
{
	...
	case DISPID_DOCUMENTCOMPLETE:
		{
			VARIANT *vt = (VARIANT*)pDispParams->rgvarg->bstrVal;
			CString str(*vt);
	
			OnDocumentCompleted(str.GetBuffer());
		}
		break;
	...
	case DISPID_AMBIENT_DLCONTROL:
		...
		pVarResult->vt = VT_I4;
		pVarResult->lVal = DLCTL_NO_JAVA |   DLCTL_NO_DLACTIVEXCTLS; 
...
void CWebBrowserBase::OnDocumentCompleted(LPCTSTR lpURL)
{
}
...
IHTMLDocument3 *CWebBrowserBase::GetDocument3()
{
	...
	if (_pWB2 != NULL) {
		...
		HRESULT hr = _pWB2->get_Document(&document_dispatch);
...
IHTMLDocument2 *CWebBrowserBase::GetDocument2()
{
...
BOOL CWebBrowserBase::GetCookie(CString& refString)
{
	BSTR bstrCookie;
